...
describe('TextFormatPipe', () => {
    ...
    it('should highlight keywords', () => {
        const result = pipe.transform('Highlight this word.', ['word']) as SafeHtml & {
            changingThisBreaksApplicationSecurity: string;
        };
        expect(result.changingThisBreaksApplicationSecurity).toContain(
            'Highlight this <span class="highlight">word</span>.',
        );
    });

    it('should escape HTML to prevent XSS', () => {
        const result = pipe.transform('<script>alert("XSS")</script>') as SafeHtml & {
            changingThisBreaksApplicationSecurity: string;
        };
        expect(result.changingThisBreaksApplicationSecurity).toContain(
